#!/usr/bin/env python3
"""
Minimal UART communication helper for Khadas VIM 5.

Default UART device:
  /dev/ttyS4

VIM 5 40-pin header UART pins:
  PIN15 = RX
  PIN16 = TX
"""
...
def open_uart(device: str, baud: int) -> int:
    if not os.path.exists(device):
        raise FileNotFoundError(
            f"{device} does not exist. On VIM 5 UART needs the uart_ao_e overlay enabled "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Send and receive UART data on Khadas VIM 5 /dev/ttyS4."
    )
    parser.add_argument(
        "--device",
